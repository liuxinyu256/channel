...
int main(void) {
    printf("=== Channel 框架使用示例 ===\n\n");

    /* 1. 创建定时器（注入超时回调） */
    frame_timer_t *timer = frame_timer_hw_create(
        timeout_timer_callback,  /* 超时封包器标准回调 */
        NULL,                    /* 回调上下文（填 pkt，timer 创建后再设） */
        10000,                   /* 10ms 超时 */
        0                        /* 硬件定时器通道 0 */
    ...
    /* 3. 设置帧完成回调 */
    set_frame_finish_callback(pkt, NULL);  /* 实际填入 channel 层回调 */
    printf("[OK] 帧完成回调已设置\n");

    /* 4. 模拟接收字节（硬件串口 ISR 中调用 put_byte） */
    printf("\n--- 模拟接收 ---\n");
